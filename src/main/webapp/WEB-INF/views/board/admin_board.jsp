			...
			<section id="container">
				<form role="form" method="get">
					<table class="table table-hover">
						<thead>
							<tr><th>번호</th><th>제목</th><th>작성자</th><th>등록일</th><th>조회수</th><th>삭제여부</th></tr>
						</thead>
						
						<c:forEach items="${list}" var = "list">
							<tr>
									<td><c:out value="${list.bno}" /></td>
									<td><a href="/puppy13/board/admin_readView.do?bno=${list.bno}&page=${scri.page}&perPageNum=${scri.perPageNum}&searchType=${scri.searchType}&keyword=${scri.keyword}"><c:out value="${list.title}" /></a></td>
									<td><c:out value="${list.writer}" /></td>
									<td><fmt:formatDate value="${list.regdate}" pattern="yyyy-MM-dd HH:mm"/></td>
									<td style="color: blue;"><c:out value="${list.hit}" />
										<c:if test="${list.hit >= 10000}">
											<img alt="이미지 없음." src="<%=request.getContextPath()%>/resources/image/beat10000.png" width="25" height="25">
										</c:if>
										<c:if test="${list.hit >= 1000}">
											<img alt="이미지 없음." src="<%=request.getContextPath()%>/resources/image/beat1000.png" width="25" height="25">
										</c:if>
										<c:if test="${list.hit >= 100}">
											<img alt="이미지 없음." src="<%=request.getContextPath()%>/resources/image/beat100.png" width="25" height="25">
										</c:if>
									</td>
									<td>
										<c:if test="${list.del_gb == 'N'}">
											<b style="color: blue"><c:out value="${list.del_gb}" /></b>
										</c:if>
										<c:if test="${list.del_gb == 'Y'}">
											<b style="color: red"><c:out value="${list.del_gb}" /></b>
										</c:if>
					...
					<div class="col-md-offset-3" align="center">
						...
						<ul class="pagination">
							<c:if test="${pageMaker.prev}">
								<li><a href="admin_board.do${pageMaker.makeSearch(pageMaker.startPage - 1)}">이전</a></li>
								&nbsp;&nbsp;&nbsp;&nbsp;
							</c:if> 
							
							<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
								<li <c:out value="${pageMaker.cri.page == idx ? 'class=info' : ''}" />>
								<a href="admin_board.do${pageMaker.makeSearch(idx)}">${idx}</a></li>
								&nbsp;&nbsp;&nbsp;&nbsp;
							</c:forEach>
							
							<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
								<li><a href="admin_board.do${pageMaker.makeSearch(pageMaker.endPage + 1)}">다음</a></li>
								&nbsp;&nbsp;&nbsp;&nbsp;
							</c:if> 
						</ul>
